import type { CheckboxState, StateSettings, MultistateCheckboxesSettings } from "./types";

// ─── Все состояния по умолчанию ─────────────────────────────────────────────

export const ALL_STATES: CheckboxState[] = [
	{
		task: " ",
		name: "Todo",
		defaultColor: null,
...
export const STATE_MAP: Record<string, CheckboxState> = {};
for (const s of ALL_STATES) {
	STATE_MAP[s.task] = s;
}

// Дефолтный порядок цикла
export const DEFAULT_CYCLE_ORDER = " />!*\"?liIfkudwpcb ";

// ─── Дефолтные настройки ─────────────────────────────────────────────────────

export function defaultStateSettings(task: string): StateSettings {
	return {
		enabled: task === " " || task === "x" || task === "-" ? true : false,
	};
}

export function makeDefaultSettings(): MultistateCheckboxesSettings {
